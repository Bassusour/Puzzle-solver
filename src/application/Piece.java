...
public class Piece extends Polygon implements Comparable<Piece> {
	...
	private double groupedPivotPointX;
	private double groupedPivotPointY;
	...
	public double getCenterX() {
		double avg = 0;
		for (int i = 0; i < this.getPoints().size(); i += 2) {
			avg += this.getLocalToSceneTransform().transform(this.getPoints().get(i), this.getPoints().get(i + 1))
					.getX();
		}
		avg = avg / (this.getPoints().size() / 2);
		return avg;
	...
	public double getCenterY() {
		double avg = 0;
		for (int i = 1; i < this.getPoints().size(); i += 2) {
			avg += this.getLocalToSceneTransform().transform(this.getPoints().get(i - 1), this.getPoints().get(i))
					.getY();
		}
		avg = avg / (this.getPoints().size() / 2);
		return avg;
	...
	private void calculateValues(ObservableList<Double> list) {
		...
		// Re-order array, so the first element corresponds to the smallest value of the
		// angle
...
//		System.out.println();
	...
	public void updatePointsRotate(double degrees) {
		...
		for (int i = 0; i < this.getPoints().size(); i += 2) {
			double oldX = (this.getPoints().get(i) + this.getTranslateX())
					- (this.getLayoutBounds().getCenterX() + this.getTranslateX());
			double oldY = (this.getPoints().get(i + 1) + this.getTranslateY())
					- (this.getLayoutBounds().getCenterY() + this.getTranslateY());
	...
	public void updateGroupRotate(double degrees, Group group) {
		...
		for (int i = 0; i < this.getPoints().size(); i += 2) {
			...
			double oldX = (this.getPoints().get(i) + group.getTranslateX() + this.getTranslateX())
					- (group.getLayoutBounds().getCenterX() + group.getTranslateX());
			double oldY = (this.getPoints().get(i + 1) + group.getTranslateY() + this.getTranslateY())
					- (group.getLayoutBounds().getCenterY() + group.getTranslateY());
	...
	public double getGroupedPivotPointX() {
		return groupedPivotPointX;
	}

	public void setGroupedPivotPointX(double pivotX) {
		groupedPivotPointX = pivotX;
	}

	public double getGroupedPivotPointY() {
		return groupedPivotPointY;
	}

	public void setGroupedPivotPointY(double pivotY) {
		groupedPivotPointY = pivotY;
	}
